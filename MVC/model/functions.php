...
function check_email_secur($email, $edit_id){
    $pdo = connect_to_db();
    $sql = "SELECT * FROM users WHERE :email=email";
    $state = $pdo->prepare($sql);
    $state->execute(["email"=>$email]);
      $email_chek = $state->fetch(PDO::FETCH_ASSOC);
    if (!empty($email_chek)){
        message('danger_email_add_secur',"Почта ".$email." занята");
        redirect_to("../view/security.php?id=".$edit_id);
    }else return true;
}
...
function check_access_to_edit_user($logged_id, $edit_id){
    ...
    elseif (!isset($_SESSION['admin'])) {
        if ($logged_id!=$edit_id){
            redirect_to('../view/users.php?id='.$edit_id);
...
function upload_avatar($path_img, $img, $user_id){
    ...
    $path_img = "../img/demo/avatars/".$img_name;
